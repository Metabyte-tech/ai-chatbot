...
export default function Page() {
  ...
  const handleEmailContinue = (e: React.FormEvent) => {
    ...
    // Route to register page with prefilled email for new users
    router.push(`/register?email=${encodeURIComponent(email)}`);
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="relative flex w-full max-w-3xl overflow-hidden rounded-2xl shadow-2xl">
        {/* Close button */}
        <button
          type="button"
          onClick={() => router.back()}
          className="absolute top-3 right-3 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-white/80 text-zinc-500 shadow hover:bg-white hover:text-zinc-900 transition-colors"
          aria-label="Close"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="hidden md:flex w-[46%] flex-col items-start justify-between bg-gradient-to-br from-[#0a1628] via-[#0f2a5e] to-[#0d3b4e] px-10 py-10 text-white relative rounded-l-2xl">
          <div className="absolute inset-0 bg-gradient-to-b from-teal-400/20 via-transparent to-transparent rounded-l-2xl pointer-events-none" />
        ...
        <div className="flex flex-1 flex-col bg-white px-8 py-10 md:px-10 rounded-r-2xl">
          ...
          <div className="mt-8 flex flex-col gap-3">
            ...
            {/* Alibaba.com — not supported as no public OAuth */}
            <button
              type="button"
              disabled
          ...
          <form onSubmit={handleEmailContinue} className="mt-4 flex flex-col gap-3">
            ...
            <button
              ...
              className="h-11 w-full rounded-lg bg-zinc-900 text-sm font-semibold text-white transition hover:bg-zinc-700 disabled:opacity-60"
            >
              {isSuccessful ? "Redirecting…" : "Continue"}
            </button>
          </form>
