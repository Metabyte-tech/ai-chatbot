...
function DesignHero({ sendMessage, setInput, chatId }: any) {
    ...
    return (
        <div className="flex flex-col h-dvh bg-background overflow-hidden w-full absolute top-0 left-0 hover:z-20">
            ...
            <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-3 shrink-0 bg-white z-10 w-full relative">
                <span className="text-sm font-medium text-zinc-700">Design with AI</span>
                <div className="flex items-center gap-4">
                    <button className="flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-800 transition-colors">
                        My lists <ArrowUpRight className="h-3.5 w-3.5" />
                    </button>
                    <button className="flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-800 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-8 max-w-3xl mx-auto w-full">
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <p className="text-zinc-700 text-sm leading-relaxed">
                        No design skills needed, just{" "}
                        <span className="underline underline-offset-2 cursor-pointer hover:text-zinc-900">share your idea</span>
                        {" "}in plain word—I'll generate a product design with AI.
                    </p>

                    <div className="flex flex-col gap-3 mt-2">
                        <p className="text-sm font-semibold text-zinc-800">
                            Click a sample to start 👇
                        </p>
                        <div className="flex flex-col gap-2">
                            {SAMPLES.map((sample) => (
                                <button
                                    ...
                                    className="flex items-start gap-3 rounded-xl border border-zinc-100 bg-white px-4 py-3 text-left text-sm text-zinc-700 transition-all hover:border-purple-200 hover:bg-purple-50/50 hover:shadow-sm group"
                                >
                                    <CornerDownLeft className="mt-0.5 h-3.5 w-3.5 shrink-0 text-zinc-300 group-hover:text-purple-400 transition-colors" />
                                    {sample}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-col gap-2 mt-4 border-t border-zinc-100 pt-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex gap-0.5">
                                {[1, 2, 3, 4, 5].map((i) => (
                                    <Star key={i} className="h-3 w-3 text-zinc-200 fill-zinc-200" />
                                ))}
                            </div>
                        </div>
                    </div>
    ...
    return (
        <>
            <Chat
                ...
                isReadonly={false}
                renderCustomEmptyState={(props) => <DesignHero {...props} />}
                key={id}
            />
            <DataStreamHandler />
        </>
