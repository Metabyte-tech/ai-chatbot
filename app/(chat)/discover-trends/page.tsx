...
const SAMPLES = [
    "Wireless earbuds in Europe",
    "Eco-friendly kitchenware in the US",
...
export default function DiscoverTrendsPage() {
    return (
        <Suspense fallback={<div className="flex h-dvh" />}>
            <DiscoverTrendsChatPage />
        </Suspense>
    );
}

function DiscoverHero({ sendMessage, setInput, chatId }: any) {
    ...
    return (
        <div className="flex flex-col h-dvh bg-background overflow-hidden w-full absolute top-0 left-0 hover:z-20">
            <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-3 shrink-0 bg-white z-10 w-full relative">
                <span className="text-sm font-medium text-zinc-700">Discover trends</span>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-8 max-w-3xl mx-auto w-full">
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <p className="text-zinc-700 text-sm leading-relaxed">
                        Just tell me your <strong>product niche</strong> — I can uncover rising trends and hot-selling opportunities fast.
                    </p>
                        ...
                        <div className="flex flex-col gap-2">
                            {SAMPLES.map((sample) => (
                                <button
                                    ...
                                    className="flex items-start gap-3 rounded-xl border border-zinc-100 bg-white px-4 py-3 text-left text-sm text-zinc-700 transition-all hover:border-orange-200 hover:bg-orange-50/50 hover:shadow-sm group"
                                >
                                    <CornerDownLeft className="mt-0.5 h-3.5 w-3.5 shrink-0 text-zinc-300 group-hover:text-orange-400 transition-colors" />
                                    {sample}
                                </button>
                            ))}
                        </div>
...
function DiscoverTrendsChatPage() {
    ...
    return (
        <>
            <Chat
                ...
                isReadonly={false}
                renderCustomEmptyState={(props) => <DiscoverHero {...props} />}
                key={id}
            />
            <DataStreamHandler />
        </>
